...
columns = ("Sr. No.", "Student Name", "Order Details", "Cook Employee")
...
# Circular queue to assign orders to employees
...
def show_order_details(event):
    ...
    if selected_item:
        ...
        cook_employee = item["values"][3]
        details_text = f"Order Details:\n{order_details}\n\nCook Employee: {cook_employee}"
...
def update_cook_interface():
    ...
    # Insert retrieved data into the table with Sr. No., Name, Order Details, and Cook Employee columns
    global employee_index
    for i, (_, student_name, prn_no, department, ordered_items, total_amount, order_datetime, bill_paid) in enumerate(data, 1):
        truncated_details = ordered_items[:50] + ("..." if len(ordered_items) > 50 else "")
        cook_employee = employees[employee_index]
        bill_tree.insert("", "end", values=(i, student_name, truncated_details, cook_employee))
